...
public class ARPGcameraC : MonoBehaviour {
	...
	public float maxDistance = 12; // 最大视距
	public float minDistance = 1.0f;
	public float xSpeed = 250.0f;
	public float ySpeed = 120.0f;
	public float yMinLimit = -60; // 更向下看
	public float yMaxLimit = 120; // 更向上看
	...
	void LateUpdate(){
		...
		if(Input.GetAxis("Horizontal") != 0 || Input.GetAxis("Vertical") != 0 || target.GetComponent<AttackTriggerC>() && target.GetComponent<AttackTriggerC>().isCasting && !target.GetComponent<AttackTriggerC>().onLocking || lockOn){
