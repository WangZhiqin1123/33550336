...
public class AttackTriggerC : MonoBehaviour {
	...
	[HideInInspector]
	public bool heavyAttack = false; // 重击标志（跳跃攻击时设置）
	...
	void Update(){
		...
		//Blocking Damage - 按住右键格挡，持续消耗体力，松手立即停止
		if(GetComponent<PlayerInputControllerC>() && canBlock && Input.GetButton("Fire2") && Time.time > nextFire && !isCasting && !skillAim && !mobileMode){
			if(!GetComponent<StatusC>().block && GetComponent<PlayerInputControllerC>().stamina > 0 && !GetComponent<PlayerInputControllerC>().dodging){
				// 开始格挡，消耗初始体力
			...
			// 持续格挡中，每帧消耗少量体力
			if(GetComponent<StatusC>().block && GetComponent<PlayerInputControllerC>().stamina > 0){
				float staminaDrainRate = 1.0f; // 每秒消耗1点体力
				GetComponent<PlayerInputControllerC>().stamina -= staminaDrainRate * Time.deltaTime;
				// 体力耗尽时停止格挡
				if(GetComponent<PlayerInputControllerC>().stamina <= 0){
					GetComponent<StatusC>().GuardBreak();
				}
			}
		}
		// 松开右键立即停止格挡
		if(GetComponent<StatusC>().block && !Input.GetButton("Fire2")){
			GetComponent<StatusC>().GuardBreak();
		}
	...
	IEnumerator AttackCombo(){
		// 显示连击UI (已注释 - ComboDisplayC类已删除)
		// ComboDisplayC comboDisplay = GetComponent<ComboDisplayC>();
		// if(comboDisplay != null){
		//	comboDisplay.OnAttackEvent();
		// }
		...
		if(attackCombo[c] || useMecanim){
			...
			BulletStatusC bulletStatus = bulletShootout.GetComponent<BulletStatusC>();
			bulletStatus.Setting(str , matk , "Player" , this.gameObject);
			if(GetComponent<StatusC>().hiddenStatus.drainTouch > 0){
				bulletStatus.drainHp += GetComponent<StatusC>().hiddenStatus.drainTouch;
			}
			
			// 判断是近战还是远程攻击
			if(bulletShootout.GetComponent<Rigidbody>()){
				// 远程攻击（有刚体）：启用距离衰减
				bulletStatus.useDistanceFalloff = true;
				bulletStatus.isMeleeAttack = false;
			}else{
				// 近战攻击（无刚体）：启用格挡反击加成
				bulletStatus.useDistanceFalloff = false;
				bulletStatus.isMeleeAttack = true;
			}
			
			// 处理重击伤害（1200%）- 跳跃攻击时使用剑触发
			if(heavyAttack){
				bulletStatus.totalDamage *= 12; // 1200%伤害
				heavyAttack = false; // 重置重击标志
				Debug.Log("=== HEAVY ATTACK! Damage x12 ===");
			}
			if(c >= attackCombo.Length){
				c = 0;
				atkDelay = true;
