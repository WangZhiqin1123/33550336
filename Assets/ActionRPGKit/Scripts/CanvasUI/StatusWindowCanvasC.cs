...
public class StatusWindowCanvasC : MonoBehaviour {
	...
	private int pointMultiplierValue = 1;
	...
	void Update(){
		...
		if(pointMultiplier){
			pointMultiplier.text = "x" + pointMultiplierValue.ToString();
		}
	...
	public void UpgradeStatus(int statusId){
		...
		StatusC stat = player.GetComponent<StatusC>();
		if(statusId == 0 && stat.statusPoint >= pointMultiplierValue){
			stat.atk += pointMultiplierValue;
			stat.statusPoint -= pointMultiplierValue;
			stat.CalculateStatus();
		}
		if(statusId == 1 && stat.statusPoint >= pointMultiplierValue){
			stat.def += pointMultiplierValue;
			stat.maxHealth += 5 * pointMultiplierValue;
			stat.statusPoint -= pointMultiplierValue;
			stat.CalculateStatus();
		}
		if(statusId == 2 && stat.statusPoint >= pointMultiplierValue){
			stat.matk += pointMultiplierValue;
			stat.maxMana += 3 * pointMultiplierValue;
			stat.statusPoint -= pointMultiplierValue;
			stat.CalculateStatus();
		}
		if(statusId == 3 && stat.statusPoint >= pointMultiplierValue){
			stat.mdef += pointMultiplierValue;
			stat.statusPoint -= pointMultiplierValue;
			stat.CalculateStatus();
		}
	}
	...
	public void MultiplyPoint(){
		if(pointMultiplierValue < 10000){
			pointMultiplierValue *= 10;
			if(pointMultiplierValue > 10000){
				pointMultiplierValue = 10000;
			}
		}
	}
	
	public void DividePoint(){
		if(pointMultiplierValue > 1){
			pointMultiplierValue /= 10;
			if(pointMultiplierValue < 1){
