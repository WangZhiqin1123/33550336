...
public class StatusC : MonoBehaviour {
	...
	[HideInInspector]
	public float lastGuardTime = 0f;
	public float counterAttackWindow = 1f;
	[HideInInspector]
	public int counterAttackStacks = 0;
	public int maxCounterAttackStacks = 3;
	...
	public int outOfCombatHealAmount = 10;
	...
	void Start(){
		...
		if(hiddenStatus == null){
			hiddenStatus = new HiddenStat();
		}
		if(hiddenStatus.counterAttackBonus == 0){
			hiddenStatus.counterAttackBonus = 200;
		}
		CalculateStatus();
	}
	...
	public string OnDamage(int amount , int element){
		if(!dead){
			...
			if(block){
				...
				counterAttackStacks = maxCounterAttackStacks;
				Debug.Log("=== GUARD SUCCESS! Counter stacks set to: " + counterAttackStacks + " ===");
				return "Guard";
			}
	...
	public bool IsInCounterAttackWindow(){
		if(Time.time - lastGuardTime <= counterAttackWindow){
			return true;
		}
		return false;
	}
	...
	void Update(){
		...
		// 检查是否脱战
		if(isInCombat){
			if(Time.time - lastCombatTime >= outOfCombatDelay){
				isInCombat = false;
			}
		}
		...
		// 脱战状态下每秒回复
		if(!isInCombat){
			health += (int)(outOfCombatHealAmount * Time.deltaTime);
			mana += (int)(outOfCombatHealAmount * Time.deltaTime);
...
public class HiddenStat{
	...
	public int counterAttackBonus = 0;
}
